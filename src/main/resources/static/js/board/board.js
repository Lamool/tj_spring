...
// 페이지 정보들을 관리하는 객체, 전역변수, 함수의 매개변수로 관리가 힘들어서
let pageInfo = {
    page : 1,               // 현재 페이지[기본값 1페이지]
    bcno : 0,               // 현재 카테고리[기본값 0 전체보기]
    searchKey : 'btitle',   // 현재검색필드[기본값:제목필드]
    searchKeyword : ''      // 현재검색값[기본값:공백]
}
...
doBoardFindAll(1);
function doBoardFindAll(page) {     console.log("boardPrint()");
...
function onCategory(bcno) {
    ...
    pageInfo.bcno = bcno;   // 1. 전역변수에 bcno 대입
    console.log('카테고리 변경');
