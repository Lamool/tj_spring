...
function doSignup() {
    ...
    // 3. ajax (jquery 라이브러리 필요), ajax는 비동기 통신
    $.ajax({
        async : false,               // async : true 비동기(기본값), false : 동기식  ((순차 처리 할지 말지 / true는 기본값이라 안 넣어줘도 된다))
...
function pwCheck(){ console.log("pwCheck()");
    ...
    pwCheckBox.innerHTML =`영대소문자와 숫자 조합의 5~30 글자 사이 가능합니다.`
    checkArray[1] = false;
} // method end
...
function emailCheck(){
    ...
    if( emailReg.test(email) ){
        ...
        // 이메일 중복검사 생략
        // 이메일 인증검사
            authBtn.disabled = false;   // 인증 요청 버튼 활성화
    }else{
...
function doAuth() {     console.log('doAuth()');
    ...
    authBtn.disabled = true;    // 인증요청을 계속 보내면 타이머 여러 개가 작동되므로 버튼을 누를 수 없게 막음
    // 1. 인증 번호 입력 구역 구성
...
function doAuthCode() {
    ...
    // * 임의의 인증 번호 (JS에서 인증번호를 관리하지 않는 이유 : JS는 클라이언트로부터 오픈 코드이기 때문에)
    $.ajax({
        async : false,
        method : "post",
        url : "/auth/check",
